...
while (my $line = <INF>){
	 ...
	 if ($start > $end) {
		 ...
		 if (exists $seq{$str}{$chr}) {
			 my $seqs = $seq{$str}{$chr};
			 my $start1 = $start - 1;
			 my $part = substr($seqs,$start1, $end-$start1); #from the 200bp before the start to the end
	      ...
	      if (exists $seq{$str}{$chr}) {
			  my $seqs = $seq{$str}{$chr};
			  my $start1 = $start - 1;
			  my $part = substr($seqs,$start1, $end-$start1); #from the start to 200bp after the end
			  print OUT ">$str" . "\t" . $loca . "\n$part\n";
		  }else{
			  print print ">$str" . "\t" . $loca . "\n";
			  }
	  }
}
close IN;
